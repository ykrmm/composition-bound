"""
LoRA QA fine-tuning on a pretrained Graph-BioS model (multi-hop version).

Differences vs finetune_qa.py:
  - qa_train may be a list of JSONL files (e.g. [qa_1hop_train, qa_2hop_train]).
  - Records are grouped by individual id across ALL hop files. Group-shuffle
    keeps every hop for one person contiguous so the batch is coherent.
  - Otherwise identical: same LoRA modules (c_attn QV + wte), same Allen-Zhu
    Appendix D hyperparameters.

Usage:
    python finetune_graph_qa.py --config configs/finetune_qa_graph_..._1hop2hop.yaml
"""
...
lr_schedule      = fcfg.get("lr_schedule", "cosine")

qa_train = fcfg["qa_train"]
if isinstance(qa_train, str):
    qa_train = [qa_train]
assert isinstance(qa_train, list) and len(qa_train) > 0, \
    "qa_train must be a non-empty string or list of paths"
...
optimizer = torch.optim.AdamW(
    lora_params, lr=max_lr, betas=(0.9, 0.95),
    weight_decay=weight_decay, eps=eps,
)

# -- Multi-hop QA dataloader ----------------------------------------------
...
class GraphQADataLoader:
    """Loads one or more QA JSONL files and groups all records by individual id.

    Records for one person across all hop files stay contiguous in the flat
    sample list, so a batch stride covers a small number of persons rather
    than averaging over many different ones (same spirit as Allen-Zhu).
    """
    def __init__(self, paths, batch_size):
        ...
        for path in paths:
            ...
            with open(path) as f:
                for line in f:
                    ...
                    n_file += 1
            hop_counts[path] = n_file
            print(f"  loaded {n_file:,} records from {path}")
        ...
        print(f"total {total:,} records across {len(paths)} file(s); "
              f"{len(self.groups):,} individuals; "
              f"records/person: min={min(sizes)} max={max(sizes)} "
...
loader = GraphQADataLoader(qa_train, batch_size)
