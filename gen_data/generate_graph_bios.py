"""
Graph-BioS dataset generation for multi-hop reasoning experiments.

Extends the Allen-Zhu bioS setup with two Person->Person relational edges
(friend, enemy), enabling evaluation of 2-hop and 3-hop QA. Fullname is
forced everywhere (no pronouns) so that both the subject and the
friend/enemy targets are always referred to by their full name.

Edge routing is SAME-SPLIT for rigor: P_train individuals (ids 0..49999)
only point to other P_train individuals; P_test (50000..99999) only to
other P_test. This ensures no cross-split information leakage through
multi-hop answers during QA finetune.

Usage:
    python generate_graph_bios.py --n_individuals 100000 --n_multi 5 \
        --seed 42 --out_dir graph_bios_data

Output:
    graph_bios_data/
        individuals.json                 -- 100K people with friend_id, enemy_id
        bios_single_fullname.txt         -- bioS_single (with friend/enemy sentences)
        bios_multi5p_fullname.txt        -- bioS_multi5+permute (with friend/enemy)
"""
...
# ---------------------------------------------------------------------------
# Graph edge assignment and friend/enemy sentence templates
# ---------------------------------------------------------------------------

def assign_edges(individuals, split_size=None):
    """Assign friend_id and enemy_id to each individual, SAME-SPLIT routed.

    ids 0..split_size-1        -> friend/enemy drawn from the same range
    ids split_size..n-1        -> friend/enemy drawn from the same range

    Constraints: friend_id != self, enemy_id != self, friend_id != enemy_id.
    """
    n = len(individuals)
    ...
    for i, person in enumerate(individuals):
        if i < split_size:
            lo, hi = 0, split_size - 1
        else:
            lo, hi = split_size, n - 1

        while True:
            friend_id = random.randint(lo, hi)
            enemy_id  = random.randint(lo, hi)
            if friend_id != i and enemy_id != i and friend_id != enemy_id:
                break
        person['friend_id'] = friend_id
        person['enemy_id']  = enemy_id


FRIEND_TEMPLATES = [
    "{name}'s closest friend is {friend}.",
    "{name}'s best friend is {friend}.",
    "{name} considers {friend} to be their best friend.",
    "{name} is close friends with {friend}.",
    "The closest friend of {name} is {friend}.",
    "{name} shares a close friendship with {friend}.",
    "{name} counts {friend} among their closest friends.",
    "{name} has {friend} as their closest friend.",
    "{name} names {friend} as their best friend.",
    "{friend} is the best friend of {name}.",
    "{friend} is regarded by {name} as their closest friend.",
    "Among their friends, {name} is closest to {friend}.",
    "{name} has developed a strong friendship with {friend}.",
    "{name} and {friend} share a deep bond of friendship.",
    "{name}'s dearest friend is {friend}.",
]


ENEMY_TEMPLATES = [
    "{name}'s worst enemy is {enemy}.",
    "{name}'s enemy is {enemy}.",
    "{name} considers {enemy} to be their enemy.",
    "{name} regards {enemy} as an enemy.",
    "The worst enemy of {name} is {enemy}.",
    "{name} has a long-standing rivalry with {enemy}.",
    "{name} counts {enemy} among their enemies.",
    "{name} names {enemy} as their worst enemy.",
    "{enemy} is the worst enemy of {name}.",
    "{enemy} is regarded by {name} as an enemy.",
    "Among their adversaries, {name} considers {enemy} the worst.",
    "{name} and {enemy} are bitter enemies.",
    "{name} treats {enemy} as their worst enemy.",
    "{name}'s most hated enemy is {enemy}.",
    "{name} holds {enemy} as their worst enemy.",
]
...
def append_graph_sentences(bio_text, person, individuals):
    """Append friend/enemy sentences (fullname on both sides) to a bio.

    The bio produced by get_text_simple3 ends with a period. We strip it,
    append ". <friend_sentence> <enemy_sentence>" and rely on
    augmentation_permutation2 to shuffle all sentences together.
    """
    friend = individuals[person['friend_id']]
    enemy  = individuals[person['enemy_id']]

    subj_name  = _fullname(person)
    friend_name = _fullname(friend)
    enemy_name  = _fullname(enemy)

    f_sent = random.choice(FRIEND_TEMPLATES).format(name=subj_name, friend=friend_name)
    e_sent = random.choice(ENEMY_TEMPLATES).format(name=subj_name, enemy=enemy_name)

    bio = bio_text.rstrip()
    if bio.endswith('.'):
        bio = bio[:-1]
    return f"{bio}. {f_sent} {e_sent}"
...
def build_bios_single(individuals):
    """One biography per person, no permutation. Fullname everywhere."""
    texts = []
    for person in tqdm(individuals, desc='bios_single'):
        bio = get_text_simple3(person, fullname=True)
        bio = append_graph_sentences(bio, person, individuals)
        texts.append(bio.strip())
    return texts


def build_bios_multi_permute(individuals, n_multi=5):
    """n_multi permuted biographies per person. Fullname everywhere.

    friend/enemy sentences are appended BEFORE augmentation_permutation2,
    so they are shuffled together with the 6 base sentences.
    """
    texts = []
    for person in tqdm(individuals, desc=f'bios_multi{n_multi}p'):
...
def main():
    ...
    parser.add_argument('--out_dir',          type=str, default='../graph_bios_data')
    parser.add_argument('--from_individuals', type=str, default=None,
                        help='Path to existing individuals.json — skips resampling. '
                             'If the file lacks friend_id/enemy_id, edges will be assigned.')
    ...
    # 2. Assign friend/enemy edges (same-split routed)
    has_edges = individuals and 'friend_id' in individuals[0] and 'enemy_id' in individuals[0]
    if not has_edges:
        print(f"\nAssigning friend/enemy edges (same-split, split_size={len(individuals)//2:,})...")
        assign_edges(individuals)
        ind_path = os.path.join(args.out_dir, 'individuals.json')
        with open(ind_path, 'w') as f:
            json.dump(individuals, f)
        print(f"  saved → {ind_path}")
    else:
        print("  friend_id/enemy_id already present — skipping edge assignment")

    # 3. bioS_single (fullname everywhere)
    print("\nGenerating bioS_single_fullname...")
    single_texts = build_bios_single(individuals)
    random.shuffle(single_texts)
    print(f"  shuffled {len(single_texts):,} bios")
    write_dataset(single_texts, os.path.join(args.out_dir, 'bios_single_fullname_all.txt'))

    # 4. bioS_multi5+permute (fullname everywhere)
    print(f"\nGenerating bioS_multi{args.n_multi}p_fullname...")
    multi_texts = build_bios_multi_permute(individuals, args.n_multi)
    random.shuffle(multi_texts)
    print(f"  shuffled {len(multi_texts):,} bios")
    write_dataset(multi_texts, os.path.join(args.out_dir, f'bios_multi{args.n_multi}p_fullname_all.txt'))
